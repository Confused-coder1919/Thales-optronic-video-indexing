...
## Manual Update (Current State)

- **Repo path**: `/Users/syedtashfin/Documents/GitHub/Thales-optronic-video-indexing` (git repo, branch `main`).
- **Primary goal**: build a searchable intelligence index from video by fusing STT + entity extraction + vision verification into reports and pivot timelines.
- **Pipeline flow**: video upload → audio extraction → STT (Whisper) → entity extraction (Mistral) → vision verification (Pixtral) → fused JSONL + reports + optional CSV.
- **Discovery mode (optional)**: vision-first entity proposals sampled from frames (`THALES_DISCOVERY_MODE=1` or UI toggle), merged with speech entities before verification. Adds optional per-entity `source` + `discovered_only` fields in `video_<id>_report.json`. UI surfaces a "Vision-only entities" table when present.
- **Key outputs**:
  - `reports_ui/summary_report.json`, `reports_ui/video_<id>_report.json`
  - `reports_ui/pivot/*_speech.jsonl`, `*_vision.jsonl`, `*_merged.jsonl`
  - `reports_ui/thales_metadata.csv` (optional)
  - `backend/data/output/audio_<id>_<timestamp>/segments.csv` + `sitrep.json`
  - `data/voice_<id>.txt` (generated transcript for entity extraction)
- **Streamlit UI** (`ui/app.py`): single-video upload, auto audio extraction, pipeline timeline + run metadata, model config panel, key findings, results tables, AI scene timeline, frame-by-frame detections, transcript search, analysis outputs, downloads, FAQ, and a sticky in-page navigation bar.
- **Scene analysis**: optional Pixtral scene summaries saved as `reports_ui/<stem>_scene.json`.
- **Key modules**: `thales/discovery.py`, `thales/scene_analysis.py`, `thales/pivot.py`, `thales/fusion.py`, `thales/postprocess.py`, `thales/export_thales_csv.py`, `ui/app.py`, `scripts/check_discovery_output.py`.
- **UI cleanup**: removed redundant deliverables cards and “How the pipeline works” section; transcript view now falls back to `segments.csv` if `voice_<id>.txt` is missing.
- **Security**: API keys must be provided via Streamlit secrets or environment variables; **do not** commit keys.
- **Deployment**: Streamlit Cloud requires a “Reboot app” to pick up UI changes.

## Repo Metadata

Git repository available (branch: `main`).
...
Note: The editable install path in this snapshot may be stale; current repo path is `/Users/syedtashfin/Documents/GitHub/Thales-optronic-video-indexing`.
...
Note: The README excerpt below is legacy; current entry points are `python -m thales` (CLI)
and `streamlit run ui/app.py` (UI). See **Manual Update (Current State)** for the latest flow.
...
- Projet académique Thales – _Video Indexing Pipeline_
...
export MISTRAL_API_KEY=YOUR_API_KEY  # or set it in .env
...
Note: The UI has been heavily updated since this snapshot (new pipeline timeline, analysis outputs, sticky navigation, scene analysis, search UX). Always refer to `ui/app.py` in the repo for the current implementation.
...
- `MISTRAL_API_KEY` is required for entity extraction and Pixtral vision analysis.
- First real run downloads large Hugging Face models and needs internet access.
- Streamlit Cloud may require a “Reboot app” to pick up UI changes.
